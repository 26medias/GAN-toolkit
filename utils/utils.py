...
def get_src_inputs(fn, fd, fp, idet):
    ...
    Inputs:
        ...
        idet: An instance of IrisDetector
    Outputs:
...
def get_tar_inputs(fns, fd, fv):
    """
    Inputs:
        fn: A string or a list. Path to an image file(s).
        fd: An instance of FaceAlignmentDetector. Face detector in face_toolbox_keras. 
    ...
    if not type(fns) == list:
        if type(fns) == str:
            ...
            raise ValueError("Received and unknown filename type. fns should be a list or a string.")
